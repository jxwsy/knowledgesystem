...
获取数据：[https://griffin.apache.org/data/streaming/](https://griffin.apache.org/data/streaming/)
      ...
      "connectors": [
        {
          ...
          "config": {
            "kafka.config": {
              "bootstrap.servers": "bigdata101:9092",
              "group.id": "griffin",
  ...
  "griffin.checkpoint": [
    {
      "type": "zk",
      "config": {
        "hosts": "bigdata101:2181,bigdata101:2182,bigdata103:2181",
        "namespace": "griffin/infocache",
        "lock.path": "lock",
        "mode": "persist",
        "init.clear": true,
...
出现 `java.io.CharConversionException: Invalid UTF-32 character 0x8000000(above 10ffff)  at char #1, byte #7)`，初步判断是数据被压缩成 lzo 格式，读取失败。【待解决】
...
在 hadoop 的 core-site.xml 文件中，配置了 lzo 压缩。把 `hadoop-lzo-0.4.21-SNAPSHOT.jar` 文件分别导入 hive 和 spark 中，还是不行。

暂时删除 core-site.xml 文件中 lzo 压缩配置。
